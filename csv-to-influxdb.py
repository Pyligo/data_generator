...
def loadCsv(inputfilename, servername, user, password, dbname, metric, 
    ...
    # set timestamp to loop 
    timestart = "2020-06-15T00:00:00Z"
    datetime_naive = datetime.datetime.strptime(timestart,timeformat)
    ...
    time_start = unix_time_millis(datetime_local)
    
    # open csv
       
    while time_start < time.time() * 1000:
        with open(inputfilename, 'r') as csvfile:
            ...
            count = 0
            for row in reader:
                fieldcolumns = row.keys()
                timestamp = time_start * 1000000 # in nanoseconds
                time_start += 1000
    ...
    while True:
        ...
        with open(inputfilename, 'r') as csvfile:
            ...
            for row in reader:
                fieldcolumns = row.keys()
                timestamp = int(time.time() * 1000) * 1000000 # in nanoseconds
                ...
                response = client.write_points([point])
                if response == False:
                    print('Problem inserting points, exiting...')
                    exit(1)
                print("wrote one ~~~~~~~~")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--password', nargs='?', default='liqian',
                        help='Password.')
    ...
    parser.add_argument('-tc', '--timecolumn', nargs='?', default='readable_time',
                        help='Timestamp column name. Default: timestamp.')

    parser.add_argument('-tf', '--timeformat', nargs='?', default='%Y-%m-%dT%H:%M:%SZ',
